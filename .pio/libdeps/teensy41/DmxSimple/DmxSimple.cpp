...
/** dmxBuffer contains a software copy of all the DMX channels.
  */
volatile uint32_t dmxBuffer[DMX_SIZE];
static uint16_t dmxMax = 16; /* Default to sending the first 16 channels */
static uint32_t dmxStarted = 0;
static uint16_t dmxState = 0;

static volatile uint32_t *dmxPort;
static uint32_t dmxBit = 0;
static uint32_t dmxPin = 3; // Defaults to output on pin 3 to support Tinker.it! DMX shield

void dmxBegin();
void dmxEnd();
void dmxSendByte(volatile uint32_t);
void dmxWrite(int,uint32_t);
 ...
 */
#if defined(__AVR__)
void dmxSendByte(volatile uint32_t value)
{
  uint32_t bitCount, delCount;
  __asm__ volatile (
    "cli\n"
...
void dmxSendByte(uint32_t value)
{
...
void dmxWrite(int channel, uint32_t value) {
 ...
 */
void DmxSimpleClass::usePin(uint32_t pin) {
 ...
 */
void DmxSimpleClass::write(int address, uint32_t value)
{
